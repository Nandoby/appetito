<header class="p-6">
    <div class="container">

        <nav>
            <div id="brand">
                <span class="text-logo">Appetito</span>
                <a class="logo-anim" href="/"><img src="logo.svg" alt="" class="logo"></a>
            </div>
            <form>
                <input type="search" placeholder="Recherche des recettes ou ingrédients ..."/>
            </form>
            <ul  id="auth">
                <li><a class="mr-4 animate" href="#">Connexion</a></li>
                <li><a class="mr-4 animate" href="#">Inscription</a></li>
                <li><a class="flag" href="#"><img src="flag-FR.svg" alt="Français"></a></li>
                <li><a class="ml-4 flag" href="#"><img src="flag-IT.svg" alt="Italiano"></a></li>
            </ul>
            <i id="menu-bars" class="fa-solid fa-bars"></i>
        </nav>
    </div>

</header>
